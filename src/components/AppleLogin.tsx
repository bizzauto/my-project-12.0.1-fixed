...
const AppleLogin: React.FC<AppleLoginProps> = ({ onError, className }) => {
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center gap-2 w-full max-w-[240px] h-10 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors disabled:opacity-50 ${className || ''}`}
    >
      {isSigningIn ? (
        ...
        <>
          ...
          <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Sign in with Apple</span>
        </>
      )}
    </button>
  );
