    ...
    <div className={`bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6 border ${
      ...
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 rounded-xl ${isLow ? 'bg-red-50 dark:bg-red-900/30' : 'bg-green-50 dark:bg-green-900/30'}`}>
            <WalletIcon size={20} className={isLow ? 'text-red-500' : 'text-green-500'} />
          </div>
          <div>
      ...
      <div className="flex gap-3">
        <button
          onClick={onAddFunds}
          className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl hover:from-green-600 hover:to-emerald-600 transition-all shadow-lg shadow-green-500/20 text-sm font-medium"
        >
          <Plus size={16} /> Add Funds
        </button>
        <button
          onClick={onViewHistory}
          className="px-4 py-2.5 text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 rounded-xl hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors text-sm font-medium"
        >
          History
        </button>
      </div>
